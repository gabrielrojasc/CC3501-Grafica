...
import math
from auxiliarT5 import createGPUShape
...
class AABB:
  ...
  def __init__(self, center: List[float], distX: float, distY: float,
               distZ: float) -> None:
    self.distX = distX
    self.distY = distY
    self.distZ = distZ
    self.scale = tr.scale(
        distX * 2, distY * 2,
        distZ * 2)  # transformacion de scala para la gpuShape del Bounding Box
    self.set_pos(center)
  ...
  def set_pos(self, center: List[float]):
    ...
    self.pos = tr.translate(
        center[0], center[1], center[2]
    )  # transformacion de traslacion para la gpuShape del Bounding Box
    self.transform = tr.matmul([self.pos,
                                self.scale])  # Transformacion del bounding box
  ...
  def down_raycast(self, other: "AABB") -> bool:
    ...
    return (center_x > self.minX and center_x < self.maxX and
            center_z > self.minZ and center_z < self.maxZ and
            ((other.minY < self.maxY and other.minY > self.minY) or
             math.fabs(other.minY - self.minY) < 0.01))
...
class AABBList:
  ...
  def __init__(self, pipeline, color) -> None:
    ...
    self.gpuBox = createGPUShape(
        pipeline, createBoundingBox(color))  # GPUShape del bounding box

  # Funcion que dibuja la visualizacion de los bounding boces
  def drawBoundingBoxes(self, pipeline):
    glLineWidth(1)  # Se aumenta el tamaño de las lineas dibujadas
