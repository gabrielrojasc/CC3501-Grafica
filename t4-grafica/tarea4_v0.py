...
def setLights():
  ...
  # Creamos las luces para los 2 autos
  spot3 = Spotlight()
  ...
  spot3.position = np.array([2.0, 0, 5.0])
  spot3.direction = np.array([0, 0, -1])
  spot3.cutOff = np.cos(np.radians(14))
  ...
  spot4.position = np.array([2.0, 0, 5.0])
  spot4.direction = np.array([0, 0, -1])
  spot4.cutOff = np.cos(np.radians(14))
  ...
  spot5.position = np.array([2.0, 0, 5.0])
  spot5.direction = np.array([0, 0, -1])
  spot5.cutOff = np.cos(np.radians(14))
  ...
  spot6.position = np.array([2.0, 0, 5.0])
  spot6.direction = np.array([0, 0, -1])
  spot6.cutOff = np.cos(np.radians(14))
...
# Definimos las funciones requeridas para crear las curvas de bezier
def generateT(t):
...
if __name__ == "__main__":
  ...
  # Creamos las curvas de bezier para el auto
  N = 300
  R0 = np.array([[2, 0, 5.5]]).T
  R1 = np.array([[2, 0, 2.2]]).T
  R2 = np.array([[2, 0, -1.1]]).T
  ...
  R1 = np.array([[2, 0, -7.15]]).T
  R2 = np.array([[-2, 0, -7.15]]).T
  R3 = np.array([[-2, 0, -4.5]]).T
  ...
  R0 = np.array([[-2, 0, -4.5]]).T
  R1 = np.array([[-2, 0, -1.1]]).T
  R2 = np.array([[-2, 0, 2.2]]).T
  R3 = np.array([[-2, 0, 5.5]]).T

  M2 = bezierMatrix(R0, R1, R2, R3)
  bezierCurve3 = evalCurve(M2, N)

  R0 = np.array([[-2, 0, 5.5]]).T
  R1 = np.array([[-2, 0, 8.15]]).T
  R2 = np.array([[2, 0, 8.15]]).T
  R3 = np.array([[2, 0, 5.5]]).T

  M2 = bezierMatrix(R0, R1, R2, R3)
  bezierCurve4 = evalCurve(M2, N)

  C = np.concatenate((bezierCurve1, bezierCurve2, bezierCurve3, bezierCurve4),
                     axis=0)

  # Seteamos el step y angle en 0 antes de comenzar el while
  step = 0
  angle = 0
  ...
  while not glfw.window_should_close(window):
    ...
    # Revisamos que el step no supere el largo del arreglo
    if step > N * 4 - 1:
      step = 0

    # Calculamos el angulo del auto que va solo
    if step < N * 4 - 1:
    ...
    # Ubicamos y le damos direccion a las luces para ambos autos
    spotlightsPool['spot3'].position = np.array([
        coord_X + 2 - 0.1 * np.cos(-angulo), 0.1,
        coord_Z + 5 + 0.1 * np.sin(angulo)
    ])
    spotlightsPool['spot3'].direction = np.array(
        [np.cos(-np.pi / 2 - angulo), -0.1,
         np.sin(-np.pi / 2 - angulo)])

    spotlightsPool['spot4'].position = np.array([
        coord_X + 2 + 0.1 * np.cos(-angulo), 0.1,
        coord_Z + 5 - 0.1 * np.sin(angulo)
    ])
    spotlightsPool['spot4'].direction = np.array(
        [np.cos(-np.pi / 2 - angulo), -0.1,
         np.sin(-np.pi / 2 - angulo)])

    spotlightsPool['spot5'].position = np.array([
        C[step, 0] - 0.1 * np.cos(-angle), C[step, 1] + 0.1,
        C[step, 2] + 0.1 * np.sin(angle)
    ])
    spotlightsPool['spot5'].direction = np.array(
        [np.cos(np.pi / 2 - angle), -0.1,
         np.sin(np.pi / 2 - angle)])

    spotlightsPool['spot6'].position = np.array([
        C[step, 0] + 0.1 * np.cos(-angle), C[step, 1] + 0.1,
        C[step, 2] - 0.1 * np.sin(angle)
    ])
    spotlightsPool['spot6'].direction = np.array(
        [np.cos(np.pi / 2 - angle), -0.1,
         np.sin(np.pi / 2 - angle)])
    ...
    Auto.transform = tr.matmul([
        tr.translate(coord_X + 2, -0.037409, coord_Z + 5),
    ...
    # Dibujamos el auto y lo trasladamos segun las curvas de bezier que hicimos antes
    sg.drawSceneGraphNode(car2, lightPipeline, "model")
    carNode = sg.findNode(car2, "system-car")
